...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            // Validasi apakah event sold out
            $event = \App\Models\Event::findOrFail($data['event_id']);
            if ($event->isSoldOut()) {
                return response()->json([
                    'ok' => false,
                    'message' => 'Maaf, semua tiket untuk event ini sudah habis (SOLD OUT)'
                ], 422);
            }
            ...
            $order = DB::transaction(function () use ($data, $user) {
                ...
                foreach ($data['items'] as $it) {
                    ...
                    if ($t->stok < $it['jumlah']) {
                        if ($t->stok == 0) {
                            throw new \Exception("Tiket {$t->tipe} sudah SOLD OUT");
                        }
                        throw new \Exception("Stok tidak cukup untuk tiket {$t->tipe}. Tersisa: {$t->stok}");
                    }
